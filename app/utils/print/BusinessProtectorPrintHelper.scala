  ...
  def apply(userAnswers: UserAnswers, adding: Boolean, protectorName: String)(implicit
    ...
    def answerRows(mode: Mode): Seq[Option[AnswerRow]] = Seq(
      ...
      bound.yesNoQuestion(
        ...
        Some(rts.AddressUkYesNoController.onPageLoad(mode).url)
      ),
      bound.addressQuestion(
        UkAddressPage,
        "businessProtector.ukAddress",
        Some(rts.UkAddressController.onPageLoad(mode).url)
      ),
      bound.addressQuestion(
        NonUkAddressPage,
        "businessProtector.nonUkAddress",
